...
export default [
    ...
    response: (options: any): ApiResult<PageResult<Accident>> => {
      ...
      if (accidentLocation) {
        list = list.filter((item) =>
          item.incidentLocation.includes(accidentLocation)
        );
      }
      if (accidentCause) {
        list = list.filter((item) =>
          item.incidentCause.includes(accidentCause)
        );
      }
      if (responsiblePerson) {
        list = list.filter((item) =>
          item.responsiblePerson.includes(responsiblePerson)
        );
      }
      if (collectionDamageStatus) {
        list = list.filter(
          (item) => item.damageStatus === collectionDamageStatus
        );
      }
      if (status) {
      ...
      if (processingResult) {
        list = list.filter((item) =>
          item.processingResult.includes(processingResult)
        );
      }
  ...
  // 添加事故记录
  {
    ...
    response: (options: any): ApiResult<null> => {
      ...
      const id = (parseInt(accidents[0].id) + 1).toString();
      accidents.unshift({
        ...accident,
        id,
        code: mock('@string("upper", 2)') + mock('@string("number", 6)'),
        documentImage:
          accident.documentImage || mock('@image("300x200", "@color")'),
        collectionCode:
  ...
  // 查看事故记录详情
  {
    ...
    response: (options: any): ApiResult<Accident> => {
      ...
      const accident = accidents.find((item) => item.id === id);
      if (accident) {
        return {
          code: 0,
          message: 'success',
