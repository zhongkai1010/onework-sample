...
export default [
  {
    url: '/mock/api/inventory-check/check/discrepancy',
    method: 'get',
    response: ({ query }) => {
      const { limit = 10, planId, status } = query
      return {
        code: 0,
        message: 'success',
        data: {
          ...
          list: Array.from({ length: Number(limit) }, () => ({
            ...
            status: status ? Number(status) : mock('@integer(0, 3)'),
            ...
            explanation: mock('@cparagraph(1, 3)')
          }))
        }
      }
    }
