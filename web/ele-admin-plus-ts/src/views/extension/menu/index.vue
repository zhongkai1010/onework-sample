  ...
  <ele-page>
    <ele-card header="垂直菜单">
      <div style="display: flex; flex-wrap: wrap">
        <option-item label="暗色主题" :responsive="false" style="margin: 0 12px 12px 0">
          <el-switch v-model="dark" size="small" />
        </option-item>
        <option-item label="彩色图标" :responsive="false" style="margin: 0 12px 12px 0">
          <el-switch v-model="colorful" size="small" />
        </option-item>
        <option-item label="折叠菜单" :responsive="false" style="margin: 0 12px 12px 0">
    ...
    <ele-card header="紧凑菜单">
      <div style="display: flex; flex-wrap: wrap">
        ...
        <option-item label="隐藏标题" :responsive="false" style="margin: 0 12px 12px 0">
...
<script lang="ts" setup>
  ...
  const changeColor = (type?: 'box' | 'nav') => {
    const colors: Record<string, string> = {
      ...
      '2-2': '#722ed1',
      '3-1': '#eb2f96',
      '3-2': 'linear-gradient(135deg, rgb(67, 203, 255) 10%, rgb(151, 8, 204) 100%)',
      '3-3': 'linear-gradient(135deg, rgb(160, 254, 101) 10%, rgb(250, 1, 109) 100%)',
      '3-4': 'linear-gradient(to right, #f83600 0%, #f9d423 100%)'
    }
    const data = type === 'box' ? boxItems.value : type === 'nav' ? navItems.value : menuItems.value
