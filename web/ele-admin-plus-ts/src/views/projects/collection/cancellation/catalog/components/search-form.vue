<template>
  <el-form :model="formData" @keyup.enter="handleSubmit" @submit.prevent :inline="true">
    <el-form-item label="注销单号" prop="code">
      <el-input v-model="formData.code" placeholder="请输入注销单号" clearable style="width: 200px" />
    </el-form-item>
...
<script setup lang="ts">
  ...
  const handleSubmit = () => {
    ...
    const params = Object.fromEntries(Object.entries(formData).filter(([_, value]) => value !== undefined && value !== '')) as CancellationCatalogQueryParams
    emit('search', params)
  }
