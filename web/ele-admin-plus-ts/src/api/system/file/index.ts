...
export async function uploadBase64File(base64: string, fileName?: string) {
  ...
  const res = await request.post<ApiResult<FileRecord>>('/file/upload/base64', formData)
  if (res.data.code === 0 && res.data.data) {
    return res.data.data
  }
  return Promise.reject(new Error(res.data.message))
...
export async function pageFiles(params: FileRecordParam) {
  const res = await request.get<ApiResult<PageResult<FileRecord>>>('/file/page', { params })
  if (res.data.code === 0) {
    return res.data.data
  }
  return Promise.reject(new Error(res.data.message))
