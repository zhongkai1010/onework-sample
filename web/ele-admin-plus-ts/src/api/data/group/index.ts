...
export async function getGroupPage(params: GroupQueryParams) {
  const res = await request.get<ApiResult<PageResult<Group>>>('/data/group', {
    params
  })
...
export async function getAllGroups(groupName?: string) {
  const res = await request.get<ApiResult<Group[]>>('/data/group/all', {
    params: { groupName }
  })
  if (res.data.code === 0 && res.data.data) {
    return res.data.data
...
export async function getGroupCollections(params: GroupCollectionQueryParams) {
  const res = await request.get<ApiResult<PageResult<GroupCollection>>>('/data/group/collection', {
    params
  })
  if (res.data.code === 0 && res.data.data) {
    return res.data.data
...
export async function getGroupDetails(id: number) {
  const res = await request.get<ApiResult<Group>>('/data/group/details', {
    params: { id }
  })
  if (res.data.code === 0 && res.data.data) {
    return res.data.data
...
export async function addGroup(data: Omit<GroupEditParams, 'id'>) {
  const res = await request.post<ApiResult<null>>('/data/group', data)
  if (res.data.code === 0) {
...
export async function updateGroup(data: GroupEditParams) {
  const res = await request.put<ApiResult<null>>('/data/group', data)
  if (res.data.code === 0) {
...
export async function removeGroups(data: DeleteGroupParams) {
  const res = await request.delete<ApiResult<null>>('/data/group', {
    data
  })
  if (res.data.code === 0) {
...
export async function approveGroups(data: DeleteGroupParams) {
  const res = await request.post<ApiResult<null>>('/data/group/approve', data)
  if (res.data.code === 0) {
