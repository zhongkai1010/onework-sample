...
export async function addArtistWork(data: CreateArtistWorkParams) {
  const res = await request.post<ApiResult<unknown>>('/ArtistWork/add', data)
  if (res.data.code === 0) {
...
export async function getArtistWorkList(params: ArtistWorkQueryParams) {
  const res = await request.get<ApiResult<PageResult<ArtistWork>>>('/ArtistWork/page', {
    params
  })
...
export async function getArtistWorkDetails(id: number) {
  const res = await request.get<ApiResult<ArtistWork>>('/ArtistWork/details', {
    params: { id }
  })
  if (res.data.code === 0 && res.data.data) {
    return res.data.data
...
export async function updateArtistWork(data: UpdateArtistWorkParams) {
  const res = await request.put<ApiResult<unknown>>('/ArtistWork/update', data)
  if (res.data.code === 0) {
    return res.data.message
  }
  return Promise.reject(new Error(res.data.message))
...
export async function deleteArtistWork(ids: number[]) {
  const res = await request.post<ApiResult<unknown>>('/ArtistWork/delete', {
    ids
  })
  if (res.data.code === 0) {
    return res.data.message
